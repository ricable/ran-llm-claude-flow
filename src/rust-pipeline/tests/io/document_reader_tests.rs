...
use ran_document_pipeline::io::document_reader::{
    DocumentReader, ReadRequest, DocumentFormat, ReadOptions, ReadResult, DocumentMetadata
};
...
async fn test_document_reader_creation() {
    ...
    assert_eq!(std::mem::size_of::<DocumentReader>(), std::mem::size_of::<DocumentReader>());
}
...
async fn test_document_format_variants() {
    ...
    for format in formats {
        ...
        let deserialized: DocumentFormat = serde_json::from_str(&json).expect("Deserialization failed");
...
async fn test_file_size_limit() -> Result<()> {
    ...
    fs::write(&file_path, &large_content).await.expect("Failed to write test file");
...
async fn test_format_detection_pdf() -> Result<()> {
    ...
    fs::write(&file_path, pdf_content).await.expect("Failed to write test file");
    ...
    assert_eq!(result.metadata.mime_type, Some("application/pdf".to_string()));
...
async fn test_format_detection_html() -> Result<()> {
    ...
    let html_content = b"<!DOCTYPE html><html><head><title>Test</title></head><body>Content</body></html>";
    
    fs::write(&file_path, html_content).await.expect("Failed to write test file");
...
async fn test_format_detection_json() -> Result<()> {
    ...
    fs::write(&file_path, json_content).await.expect("Failed to write test file");
...
async fn test_format_detection_csv() -> Result<()> {
    ...
    fs::write(&file_path, csv_content).await.expect("Failed to write test file");
...
async fn test_format_detection_markdown() -> Result<()> {
    ...
    let markdown_content = b"# Main Title\n\n## Subtitle\n\n```rust\nfn main() {\n    println!(\"Hello\");\n}\n```";
    
    fs::write(&file_path, markdown_content).await.expect("Failed to write test file");
...
async fn test_format_detection_xml() -> Result<()> {
    ...
    fs::write(&file_path, xml_content).await.expect("Failed to write test file");
    ...
    assert_eq!(result.metadata.mime_type, Some("application/xml".to_string()));
...
async fn test_empty_file() -> Result<()> {
    ...
    fs::write(&file_path, b"").await.expect("Failed to write test file");
...
async fn test_binary_file_detection() -> Result<()> {
    ...
    fs::write(&file_path, &binary_content).await.expect("Failed to write test file");
...
async fn test_file_extension_priority() -> Result<()> {
    ...
    fs::write(&file_path, not_pdf_content).await.expect("Failed to write test file");
...
async fn test_read_request_serialization() {
    ...
    assert_eq!(deserialized.read_options.max_size_bytes, request.read_options.max_size_bytes);
}
...
async fn test_edge_case_extensions() -> Result<()> {
    ...
    for (filename, expected_format) in test_cases {
        ...
        let result = reader.read_document(request).await?;
        assert_eq!(result.detected_format, expected_format, "Failed for {}", filename);
    }
