...
use ran_document_pipeline::io::batch_processor::{
    BatchProcessor, BatchRequest, DocumentItem, BatchOptions, BatchResult, DocumentResult
};
...
async fn test_batch_processor_creation() {
    ...
    assert_eq!(std::mem::size_of::<BatchProcessor>(), std::mem::size_of::<BatchProcessor>());
}
...
async fn test_batch_processor_multiple_documents_parallel() -> Result<()> {
    ...
    let result_ids: std::collections::HashSet<Uuid> = result.results
        .iter()
        .map(|r| r.document_id)
        .collect();
...
async fn test_batch_result_metrics() -> Result<()> {
    ...
    assert_eq!(result.batch_id, batch_id);
    assert!(result.total_processing_time_ms > 0);
    assert_eq!(result.processed_count + result.failed_count, result.results.len());
...
async fn test_batch_options_serialization() {
    ...
    assert_eq!(deserialized.parallel_processing, options.parallel_processing);
...
async fn test_priority_handling() -> Result<()> {
    ...
    for priority in 0..=255u8 {
        if priority % 50 == 0 { // Test a subset to avoid too many documents
            documents.push(DocumentItem {
                document_id: Uuid::new_v4(),
