...
use ran_document_pipeline::io::file_handler::{FileHandler, FileInfo, FilePermissions};
...
async fn test_list_directory_with_files() -> Result<()> {
    ...
    fs::write(&file1_path, b"Content 1")
        .await
        .expect("Failed to write file1");
    fs::write(&file2_path, b"Content 2")
        .await
    ...
    for info in &contents {
        let name = Path::new(&info.file_path)
            .file_name()
            .unwrap()
            .to_string_lossy();
        if name == "subdir" {
...
async fn test_create_directory() -> Result<()> {
    ...
    let handler = FileHandler::new();
    handler
        .create_directory(&new_dir_path.to_string_lossy())
        .await?;
...
async fn test_create_directory_nested() -> Result<()> {
    ...
    let handler = FileHandler::new();
    handler
        .create_directory(&nested_dir_path.to_string_lossy())
        .await?;
...
async fn test_create_directory_already_exists() -> Result<()> {
    ...
    handler
        .create_directory(&dir_path.to_string_lossy())
        .await?;
...
async fn test_delete_file() -> Result<()> {
    ...
    fs::write(&file_path, b"Content to delete")
        .await
        .expect("Failed to write test file");
...
async fn test_delete_directory() -> Result<()> {
    ...
    fs::write(&file_in_dir, b"Content")
        .await
...
async fn test_file_permissions_readonly() -> Result<()> {
    ...
    fs::write(&file_path, b"Readonly content")
        .await
        .expect("Failed to write test file");
...
async fn test_empty_directory_listing() -> Result<()> {
    ...
    let contents = handler
        .list_directory(&empty_subdir.to_string_lossy())
        .await?;
...
async fn test_large_directory_listing() -> Result<()> {
    ...
    for info in &contents {
        let name = Path::new(&info.file_path)
            ...
            .unwrap()
            .to_string_lossy();
        if let Some(captures) = regex::Regex::new(r"file(\d+)\.txt")
            .unwrap()
            .captures(&name)
        {
...
async fn test_mixed_content_directory() -> Result<()> {
    ...
    fs::write(dir_path.join("file.txt"), b"File content")
        .await
        .expect("Failed to write file");
    fs::create_dir(dir_path.join("subdir"))
        .await
        .expect("Failed to create subdirectory");
    fs::write(dir_path.join("another.dat"), b"Binary data")
        .await
        .expect("Failed to write another file");
    fs::create_dir(dir_path.join("another_dir"))
        .await
        .expect("Failed to create another subdirectory");
...
async fn test_file_info_serialization() {
    ...
    assert_eq!(
        deserialized.permissions.readable,
        file_info.permissions.readable
    );
    assert_eq!(
        deserialized.permissions.writable,
        file_info.permissions.writable
    );
    assert_eq!(
        deserialized.permissions.executable,
        file_info.permissions.executable
    );
}
...
async fn test_special_characters_in_paths() -> Result<()> {
    ...
    fs::write(&special_file, b"Special content")
        .await
        .expect("Failed to write special file");

    let handler = FileHandler::new();
    let file_info = handler
        .get_file_info(&special_file.to_string_lossy())
        .await?;
