...
use ran_document_pipeline::io::memory_mapper::{
    MemoryMapper, MappingRequest, MappingResult, MappingStats
};
...
async fn test_memory_mapper_creation() {
    ...
    assert_eq!(std::mem::size_of::<MemoryMapper>(), std::mem::size_of::<MemoryMapper>());
}
...
async fn test_read_mapping() -> Result<()> {
    ...
    let data = mapper.read_mapping(mapping_result.mapping_id, 0, 10).await?;
...
async fn test_write_mapping() -> Result<()> {
    ...
    mapper.write_mapping(mapping_result.mapping_id, 0, write_data).await?;
...
async fn test_large_file_mapping() -> Result<()> {
    ...
    fs::write(&file_path, &large_content).await.expect("Failed to write large file");
...
async fn test_empty_file_mapping() -> Result<()> {
    ...
    fs::write(&file_path, b"").await.expect("Failed to create empty file");
