...
async fn test_ml_module_initialization() {
    ...
    initialize_ml_module().await.expect("ML module should initialize successfully");
    
    // Verify that all components are accessible
    let stats = model_selector::get_statistics().await.expect("Model selector should be initialized");
    assert_eq!(stats.total_selections, 0);
    
    let workload_stats = workload_analyzer::get_statistics().await.expect("Workload analyzer should be initialized");
    assert_eq!(workload_stats.total_analyses, 0);
    
    let memory_stats = memory_predictor::get_statistics().await.expect("Memory predictor should be initialized");
...
async fn test_workload_analysis_accuracy() {
    ...
    let test_cases = vec![
        (DocumentType::PlainText, 1024, 0.2, "Simple text should have low complexity"),
        (DocumentType::Pdf, 10_000_000, 0.8, "Large PDF should have high complexity"),
        (DocumentType::Technical, 500_000, 0.7, "Technical doc should have medium-high complexity"),
        (DocumentType::Standards3Gpp, 2_000_000, 0.9, "3GPP standard should have very high complexity"),
    ];
    ...
    for (doc_type, size, expected_complexity, description) in test_cases {
        ...
        assert!(
            ...
            description, expected_complexity, analysis.complexity_score
        );
        
        // Verify memory requirements are reasonable
        assert!(analysis.memory_requirements_gb > 0, "Memory requirements should be positive");
        assert!(analysis.memory_requirements_gb < 200, "Memory requirements should be reasonable (<200GB)");
        
        // Verify processing time estimates
        assert!(analysis.estimated_processing_time > Duration::from_secs(0), "Processing time should be positive");
        assert!(analysis.estimated_processing_time < Duration::from_secs(3600), "Processing time should be reasonable (<1 hour)");
    }
}
...
async fn test_model_selection_logic() {
    ...
    for (doc_type, size, complexity, priority, expected_models) in test_cases {
        ...
        let selected_model = model_selector::select_model(&request, &workload).await.unwrap();
        
        assert!(
            expected_models.contains(&selected_model),
            "For {:?} doc ({}KB, complexity {:.1}, priority {:?}), expected one of {:?}, got {:?}",
            doc_type, size / 1024, complexity, priority, 
...
async fn test_memory_prediction_accuracy() {
    ...
    let models = [Qwen3Model::Qwen3_1_7B, Qwen3Model::Qwen3_7B, Qwen3Model::Qwen3_30B];
    let request = create_test_request(DocumentType::Technical, 100_000, 0.7, Priority::Medium);
    
    for model in &models {
        let prediction = memory_predictor::predict_memory_usage(model, &request).await.unwrap();
        ...
        assert!(
            ...
            prediction.predicted_usage_gb, specs.memory_gb, model.name()
        );
        ...
        assert!(!prediction.pool_recommendations.is_empty(), "Should have pool recommendations for {}", model.name());
        assert!(!prediction.optimization_suggestions.is_empty(), "Should have optimization suggestions for {}", model.name());
    }
}
...
async fn test_bottleneck_analysis() {
    ...
    let bottlenecks = memory_predictor::analyze_bottlenecks(
        &Qwen3Model::Qwen3_30B, 
        &high_memory_request, 
        None
    ).await.unwrap();
    
    assert!(!bottlenecks.is_empty(), "High-memory scenario should identify bottlenecks");
    
    // Check for memory-related bottlenecks
    let has_memory_bottleneck = bottlenecks.iter().any(|b| matches!(
        b.bottleneck_type, 
    ...
    if bottlenecks.iter().any(|b| b.bottleneck_type != BottleneckType::None) {
        assert!(has_memory_bottleneck, "Large model + large document should show memory bottlenecks");
    }
    
    // Test simple scenario
    let simple_request = create_test_request(DocumentType::PlainText, 1024, 0.2, Priority::Low);
    let simple_bottlenecks = memory_predictor::analyze_bottlenecks(
    ...
    let severe_bottlenecks = simple_bottlenecks.iter()
        .filter(|b| b.severity > 0.7)
        .count();
    
    assert!(severe_bottlenecks <= 1, "Simple scenario should have few severe bottlenecks");
}
...
async fn test_end_to_end_processing() {
    ...
    for request in test_requests {
        ...
        if request.priority == Priority::Critical && request.complexity_score > 0.8 {
            ...
            assert!(model_specs.quality_score >= 0.8, 
        ...
        if request.document_size_bytes < 10_000 && request.complexity_score < 0.4 {
            ...
            assert!(model_specs.tokens_per_second > 100,
                   "Small simple task should use fast model");
        }
        
        // Record the result for benchmarking
        performance_benchmark::record_result(&request, &response).await.unwrap();
    }
}
...
async fn test_concurrent_processing() {
    ...
    for i in 0..10 {
        let request = create_test_request(
            if i % 2 == 0 { DocumentType::PlainText } else { DocumentType::Technical },
            1024 * (i + 1),
            0.3 + (i as f64 * 0.05),
    ...
    assert!(stats.total_selections >= 10, "Should have recorded all selections");
}
...
async fn test_performance_tracking() {
    ...
    let models = vec![Qwen3Model::Qwen3_1_7B, Qwen3Model::Qwen3_7B, Qwen3Model::Qwen3_30B];
    ...
    assert!(!comparisons.is_empty() || models.len() <= 1, "Should have comparison data or limited models");
}
...
async fn test_cache_effectiveness() {
    ...
    println!("First analysis: {:?}, Second analysis: {:?}", first_duration, second_duration);
    
    // Test memory prediction caching
    let start_time = std::time::Instant::now();
    let _prediction1 = memory_predictor::predict_memory_usage(&Qwen3Model::Qwen3_7B, &request).await.unwrap();
    let first_duration = start_time.elapsed();
    
    let start_time = std::time::Instant::now();
    let _prediction2 = memory_predictor::predict_memory_usage(&Qwen3Model::Qwen3_7B, &request).await.unwrap();
    let second_duration = start_time.elapsed();
    
    println!("First prediction: {:?}, Second prediction: {:?}", first_duration, second_duration);
...
async fn test_error_handling() {
    ...
    match result {
        Ok(analysis) => {
            ...
            assert!(analysis.complexity_score >= 0.0, "Should normalize negative complexity");
            assert!(analysis.complexity_score <= 1.0, "Should cap complexity at 1.0");
            assert!(analysis.memory_requirements_gb > 0, "Should have positive memory requirement");
        }
        Err(_) => {
