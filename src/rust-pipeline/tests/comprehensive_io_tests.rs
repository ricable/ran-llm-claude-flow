...
async fn test_io_module_integration() -> Result<()> {
    ...
    use ran_document_pipeline::io::{
        batch_processor::{BatchOptions, BatchProcessor, BatchRequest, DocumentItem},
        file_handler::FileHandler,
        memory_mapper::{MappingRequest, MemoryMapper},
        BatchConfig, DocumentReader, DocumentSource,
    };
    ...
    let file_info = file_handler
        .get_file_info(&file_path.to_string_lossy())
        .await?;
...
async fn test_io_performance_benchmarks() -> Result<()> {
    ...
    use ran_document_pipeline::io::{BatchConfig, DocumentReader, DocumentSource};
    ...
    let result = reader
        .read_document(DocumentSource::File(small_file))
        .await?;
    ...
    let start = Instant::now();
    let result = reader
        .read_document(DocumentSource::File(medium_file))
        .await?;
    ...
    let start = Instant::now();
    let result = reader
        .read_document(DocumentSource::File(large_file))
        .await?;
