...
use crate::ml::{MLRequest, DocumentType, Priority, Qwen3Model};
...
impl WorkloadAnalyzer {
    ...
    pub async fn analyze(&self, request: &MLRequest) -> Result<WorkloadAnalysis> {
        ...
        debug!("Starting workload analysis for request {}", request.request_id);
        ...
        let pattern = self.analyze_resource_pattern(request, &complexity, &resources).await?;
        
        // Recommend optimal model
        let recommended_model = self.recommend_model(request, &complexity, &resources, &pattern).await?;
        ...
        debug!("Workload analysis completed in {}ms for request {}", 
               analysis_time.as_millis(), request.request_id);
    ...
    async fn calculate_complexity(&self, request: &MLRequest) -> Result<ComplexityAnalysis> {
        ...
        let content_complexity = calculator.calculate_content_complexity(
            &request.document_type, 
            request.document_size_bytes
        );
        
        // Structure complexity based on document type
        let structure_complexity = calculator.calculate_structure_complexity(&request.document_type);
    ...
    /// Predict resource requirements
    async fn predict_resources(&self, request: &MLRequest, complexity: &ComplexityAnalysis) -> Result<ResourceRequirements> {
    ...
    async fn analyze_resource_pattern(
        ...
        let io_intensity = if request.document_size_bytes > 1024 * 1024 { // > 1MB
    ...
    async fn recommend_model(
        ...
        let historical_model = {
            ...
            history.get(&request.document_type)
                .and_then(|h| if h.success_rate > 0.8 { Some(h.optimal_model.clone()) } else { None })
        };
        ...
        let model = if request.priority == Priority::Critical || complexity.processing_complexity > 0.8 {
        ...
        debug!("Recommended model {} for request {} (complexity: {:.3}, memory: {} GB)", 
               model.name(), request.request_id, complexity.processing_complexity, resources.memory_gb);
    ...
    async fn update_historical_data(&self, request: &MLRequest, analysis: &WorkloadAnalysis) -> Result<()> {
        let mut history = self.historical_data.write();
        
        let entry = history.entry(request.document_type.clone()).or_insert(WorkloadHistory {
        ...
        entry.average_complexity = entry.average_complexity * old_weight + analysis.complexity_score * weight;
        ...
        entry.average_memory_usage = ((entry.average_memory_usage as f64 * old_weight + 
                                      analysis.memory_requirements_gb as f64 * weight) as u32);
...
impl ResourcePredictor {
    fn new() -> Self {
        ...
        model_profiles.insert(Qwen3Model::Qwen3_1_7B, ModelProfile {
            base_memory_gb: 12,
        ...
        model_profiles.insert(Qwen3Model::Qwen3_7B, ModelProfile {
            base_memory_gb: 28,
        ...
        model_profiles.insert(Qwen3Model::Qwen3_30B, ModelProfile {
            base_memory_gb: 45,
        ...
        document_profiles.insert(DocumentType::PlainText, DocumentProfile {
        ...
        document_profiles.insert(DocumentType::Pdf, DocumentProfile {
    ...
    fn estimate_tokens(&self, request: &MLRequest) -> u64 {
        let profile = self.document_profiles.get(&request.document_type)
            .cloned()
    ...
    fn calculate_base_memory_requirements(&self, doc_type: &DocumentType, tokens: u64, complexity: f64) -> u32 {
        let profile = self.document_profiles.get(doc_type)
            .cloned()
            .unwrap_or_default();
        
        let base_memory = (tokens as f64 * 0.001 * profile.memory_overhead * (1.0 + complexity)) as u32;
        base_memory.max(4) // Minimum 4GB
    }

    fn estimate_processing_time(&self, tokens: u64, complexity: f64, priority: &Priority) -> Duration {
    ...
    fn calculate_memory_with_overhead(&self, base_memory: u32, complexity: f64, doc_type: &DocumentType) -> u32 {
        let profile = self.document_profiles.get(doc_type)
            .cloned()
            .unwrap_or_default();
...
pub async fn initialize() -> Result<()> {
    ...
    WORKLOAD_ANALYZER.set(Arc::new(analyzer))
        .map_err(|_| PipelineError::Optimization("Failed to initialize workload analyzer".to_string()))?;
...
pub async fn analyze(request: &MLRequest) -> Result<WorkloadAnalysis> {
    let analyzer = WORKLOAD_ANALYZER.get()
        .ok_or_else(|| PipelineError::Optimization("Workload analyzer not initialized".to_string()))?;
