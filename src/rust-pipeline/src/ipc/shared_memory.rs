...
impl SharedMemoryManager {
    ...
    pub async fn new(total_size_bytes: usize) -> Result<Self> {
        tracing::info!(
            "Initializing shared memory manager with {} bytes",
            total_size_bytes
        );
    ...
    pub async fn create_region(&self, name: &str, size_bytes: usize) -> Result<Uuid> {
        ...
        tracing::debug!(
            "Creating shared memory region '{}' with {} bytes (aligned: {})",
            name,
            size_bytes,
            aligned_size
        );
        ...
        tracing::info!(
            "Created shared memory region '{}' with ID {}",
            name,
            region_id
        );
        Ok(region_id)
    ...
    pub async fn write_data(&self, region_id: Uuid, offset: usize, data: &[u8]) -> Result<()> {
        ...
        tracing::debug!(
            "Wrote {} bytes to region {} at offset {}",
            data.len(),
            region_id,
            offset
        );
    ...
    pub async fn read_data(
        ...
        tracing::debug!(
            "Read {} bytes from region {} at offset {}",
            length,
            region_id,
            offset
        );
    ...
    pub async fn delete_region(&self, region_id: Uuid) -> Result<()> {
        ...
        tracing::info!(
            "Deleted shared memory region '{}' with ID {}",
            region.name,
            region_id
        );
        Ok(())
    ...
    pub async fn get_stats(&self) -> Result<MemoryStats> {
        ...
        let regions: Vec<RegionInfo> = regions_read
            .iter()
            .map(|(id, region)| {
                RegionInfo {
                    id: *id,
    ...
    pub async fn cleanup(&self) -> Result<()> {
        ...
        let region_ids: Vec<Uuid> = { self.regions.read().keys().copied().collect() };
    ...
    fn allocate_aligned_memory(&self, size: usize) -> Result<NonNull<u8>> {
        unsafe {
            let layout =
                std::alloc::Layout::from_size_align(size, self.m3_max_config.alignment_bytes)
    ...
    unsafe fn deallocate_memory(&self, ptr: NonNull<u8>, size: usize) -> Result<()> {
        let layout = std::alloc::Layout::from_size_align(size, self.m3_max_config.alignment_bytes)
            .map_err(|e| {
                PipelineError::Ipc(format!("Invalid memory layout for deallocation: {}", e))
            })?;
...
pub async fn create_document_region(
...
pub async fn create_model_region(
    manager: &SharedMemoryManager,
    model_id: &str,
    size: usize,
) -> Result<Uuid> {
...
pub async fn create_results_region(
    manager: &SharedMemoryManager,
    batch_id: Uuid,
    size: usize,
) -> Result<Uuid> {
...
mod tests {
    ...
    async fn test_shared_memory_basic_operations() {
        ...
        let read_data = manager
            .read_data(region_id, 0, test_data.len())
            .await
            .unwrap();
