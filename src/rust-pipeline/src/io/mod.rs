...
impl DocumentReader {
    ...
    pub async fn read_document(&mut self, source: DocumentSource) -> Result<Vec<u8>> {
        ...
        let content = match source {
            DocumentSource::File(path) => self.read_file_optimized(&path).await?,
            DocumentSource::Directory(path) => self.read_directory_batch(&path).await?,
            DocumentSource::Zip(path) => self.read_zip_archive(&path).await?,
            DocumentSource::Stream(url) => self.read_stream(&url).await?,
    ...
    async fn read_file_mmap(&self, path: &PathBuf) -> Result<Vec<u8>> {
        ...
        let contents = std::fs::read(path).map_err(|e| PipelineError::Io(e.to_string()))?;
        Ok(contents)
    }
    ...
    async fn read_directory_batch(&self, path: &PathBuf) -> Result<Vec<u8>> {
        ...
        let contents: Vec<Vec<u8>> = stream::iter(files)
            .map(|path| async move { self.read_file_optimized(&path).await.unwrap_or_default() })
    ...
    async fn read_zip_archive(&self, path: &PathBuf) -> Result<Vec<u8>> {
        ...
        let mut archive =
            zip::ZipArchive::new(cursor).map_err(|e| PipelineError::Io(e.to_string()))?;
    ...
    async fn read_stream(&self, _url: &str) -> Result<Vec<u8>> {
...
pub fn detect_format(content: &[u8]) -> DocumentFormat {
    ...
    if content
        .windows(5)
        .any(|window| window.eq_ignore_ascii_case(b"<?xml"))
    {
