...
pub async fn generate_performance_report() -> String {
    ...
    report.push_str(&format!("System Health: {}\n", 
        if health_status { "✅ Healthy" } else { "⚠️ Issues Detected" }));
    report.push_str("\n");
    
    // M3 Max metrics
    if let Some(m3) = m3_metrics {
        let memory_usage_percent = (m3.unified_memory_used as f64 / m3.unified_memory_total as f64) * 100.0;
        report.push_str("📊 M3 Max Metrics:\n");
        report.push_str(&format!("  Memory Usage: {:.1}% ({:.1}GB / {:.1}GB)\n", 
        ...
        report.push_str(&format!("  Neural Engine: {:.1}%\n", m3.neural_engine_utilization));
    ...
    if let Some(throughput) = throughput_metrics {
        ...
        report.push_str(&format!("  Avg Processing Time: {:.1}ms\n", throughput.average_processing_time_ms));
    ...
    if !alerts.is_empty() {
        ...
        for alert in alerts.iter().take(5) { // Show only first 5 alerts
