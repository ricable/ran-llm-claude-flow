...
fn bench_model_selection(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        ("plain_text_large", DocumentType::PlainText, 1024 * 1024, 0.3),
        ("pdf_medium", DocumentType::Pdf, 100 * 1024, 0.7),
        ("pdf_large", DocumentType::Pdf, 10 * 1024 * 1024, 0.9),
        ("technical_complex", DocumentType::Technical, 500 * 1024, 0.8),
        ("standards_3gpp", DocumentType::Standards3Gpp, 2 * 1024 * 1024, 0.9),
    ];
        ...
        group.bench_with_input(
            ...
            |b, (doc_type, size, complexity)| {
                b.to_async(&rt).iter(|| async {
                    ...
                    let workload = workload_analyzer::analyze(black_box(&request)).await.unwrap();
                    black_box(workload)
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, (doc_type, size, complexity)| {
                b.to_async(&rt).iter(|| async {
                    ...
                    let selected_model = model_selector::select_model(black_box(&request), black_box(&workload)).await.unwrap();
                ...
                b.to_async(&rt).iter(|| async {
                    ...
                    for i in 0..concurrency {
                        ...
                        let handle = tokio::spawn(async move {
                            process_request(request).await.unwrap()
                        });
...
fn bench_cache_performance(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for _ in 0..5 {
            ...
            let _ = memory_predictor::predict_memory_usage(&Qwen3Model::Qwen3_7B, &test_request).await;
        }
    });

    group.bench_function("workload_analysis_cached", |b| {
        b.to_async(&rt).iter(|| async {
            let analysis = workload_analyzer::analyze(black_box(&test_request)).await.unwrap();
            black_box(analysis)
        });
    });
