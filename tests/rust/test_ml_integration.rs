...
//! 
//! Comprehensive tests for the Rust-Python ML integration pipeline

use rust_ml_integration_tests::*;
use rust_core::{types::*, ml_integration::*};
use tokio;
use uuid::Uuid;
...
async fn test_quality_enhancer() {
    ...
    let test_response = create_mock_ml_response();
    let original_qa_count = test_response.qa_pairs.len();
    ...
    assert_eq!(enhanced_response.qa_pairs.len(), original_qa_count);
    assert!(enhanced_response.semantic_quality.overall_score >= 0.0);
    
    println!("Quality enhancement test completed:");
    println!("  Original QA pairs: {}", original_qa_count);
